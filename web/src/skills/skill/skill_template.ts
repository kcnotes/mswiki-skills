...
export const getBoxTemplate = ({
    ...
    description,
    formula,
}: SkillProps, forceShowId = false) => {
    // Note: This is a naive check that will put IDs into 4th job Dual Blade skills
    const shouldShowId = forceShowId || (vSkill != null) || isHexaBoost || (Number(id[id.length - 5]) > 3);
    const header = shouldShowId
...
export const getSkillProps = (id: string, skill: ImportedSkill, getSkill: (id: string) => ImportedSkill | undefined, options?: { skillClass?: string }): SkillProps => {
    ...
    return {
        ...
        elementAttribute: getElementAttribute(skill.base.elemAttr),
        levelRequirement: skill.base.reqLev,
        maxLevel: skill.common.maxLevel,
        combatOrders: skill.base.combatOrders,
        isHexaBoost,
        vSkill: skill.base.vSkill,
        bgm: skill.base.bgm?.replace(/^.*\//g, ''),
